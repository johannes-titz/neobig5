...
## Analysis of item a144, against trust facet
...
neo_pi_r10 <- xfun::cache_rds(test_items_against_dimension(names(neo), neo, param))
...
neo_pi_r100 <- xfun::cache_rds(test_items_against_dimension(left_items, neo, param))
...
neo_pi_r1k <-xfun::cache_rds(test_items_against_dimension(left_items, neo, param))
...
neo_pi_r20k <- xfun::cache_rds(test_items_against_dimension(left_items, neo, param))
...
Sorry, this code is currently somewhat messy and not commented.
...
#pdf(file = "plots/neo_pi_r_partial_deviances2.pdf", width = 7*0.55, height = 10*0.8)
...
```{r}
res <- neo %>% 
  select(grep("C", names(neo))) %>% 
  group_by(C35) %>% 
  summarize_all(function(x) mean(x, na.rm = T)) %>% 
  slice(1:5)
table(neo$C35)
table(apply(res, 2, function(x) all(x[1] > x[2:5])))
```
